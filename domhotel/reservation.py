...
@action("reservation/reservation_history_detail" , method=["POST", "GET"])
@action("reservation/reservation_history_detail/<path:path>", method=["POST", "GET"])
@action.uses(session, db, auth, T,flash, "reservation/reservation_history_detail.html")
def reservation_history_detail(path=None):
    ...
    room = db((db.reservation.id == reservation_id) & (  db.reservation.room == db.rooms.id  ) &
            (db.rooms.category == db.rooms_category.id) &
            (db.reservation.formula == db.formula.id)
        ).select(db.rooms.number,db.rooms.description,db.rooms_category.category, 
    ...
    return dict( room=room,  T=T ,  language=language.language, username=username, controls=controls,start=start,stop=stop)
...
def reservation_history(path=None):
    ...
    if ( 'detail' not in path):
        ...
        print(query)
        #query=db.reservation.completed == True
        ...
        redirect(URL("reservation/reservation_history_detail/"+id+"/"+start+"/"+stop))
