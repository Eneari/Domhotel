    ...
    for message in messages :
        ...
        if (subcommand == None ):
            ...
            if (command == "PLUG") :
                ...
                plug_pin.value(val)
                client.publish("DOMHOTEL/H001/R"+room_number+"/STATUS/PLUG",str(val),retain=True)

            elif (command == "LIGTH") :
                print("setto LIGTH : ", str(val))
                ligth_pin.value(val)
                client.publish("DOMHOTEL/H001/R"+room_number+"/STATUS/LIGTH",str(val),retain=True)

            elif (command == "AC") :
                print("setto AC : ", str(val))
                ac_pin.value(val)
                client.publish("DOMHOTEL/H001/R"+room_number+"/STATUS/AC",str(val),retain=True)
